...
class ST_Core {
  constructor (o = {}) {
    ...
    this.$dom.body.css ({
      ...
      '--st-header-percent-height': parseFloat (
        this.sizes.header / this.sizes.win * 100
      ).toFixed (2) + '%',
      '--st-footer-height': this.sizes.footer + 'px',
      '--st-footer-percent-height': parseFloat (
        this.sizes.footer / this.sizes.win * 100
      ).toFixed (2) + '%',
...
class ST_Smooth_Scroll {
  constructor ($obj = null, core = null, options = {}) {
    ...
    this.$scroll = jQuery ('<div class="stss-container"/>').appendTo (
      this.$wrap
    );
  ...
  updateScroll (deltaTime = 0) {
    ...
    if (this.options.animateOnStop && this.state === 'stop') {
      ...
      if (
        ...
        parseFloat (this.pos.c).toFixed (1) !=
        parseFloat (this.pos.t).toFixed (1)
      ) {
        this.state = 'play';
...
class ST_SAP {
  ...
  addParallaxMedia (elem = null, options = {}) {
    if (elem === null) {
      console.warn (
        'Cannot add Parallax Media Item. Element is not specified.'
      );
      return false;
    }
    const _self = this;
    const item = {
      ...
      mouse_cx: 0,
      calculate: function () {
        let offset_fix = _self.core.scroll !== null
        ...
        this.start =
          this.$wrap.offset ().top - _self.screen_height - offset_fix;
    ...
    const item = {
      ...
      calculate: function () {
        let offset_fix = _self.core.scroll !== null
          ...
          : 0;
        this.height = this.$el.height ();
        (this.i_scale = this.$el.css ('--st-eos-init-scale') !== undefined
          ? parseFloat (this.$el.css ('--st-eos-init-scale'))
          : 0.75), (this.scale_path = 1 - this.i_scale), (this.i_clip = 50 -
          (this.$el.css ('--st-eos-init-clip') !== undefined)
          ? parseFloat (this.$el.css ('--st-eos-init-clip')) * 50
          : 12.5), (this.br = this.$el.css ('--st-eos-br') !== undefined // 12.5 = 25% vissibility or 0.25 in styles; 50 - fully collapsed, 0 - fully visible
  ...
  addTextFillMaskItem (elem = null, options = {}) {
    if (elem === null) {
      console.warn (
        'Cannot add Text Fill Mask Item. Element is not specified.'
      );
      return false;
    }
    const _self = this;
