...
class convert_Before_After {
  constructor ($obj = null) {
    ...
    this.$el = {
      $wrap: $obj,
      $before: jQuery (
        '<div class="convert-before-after-img convert-before-img"/>'
      ).appendTo ($obj),
      $after: jQuery (
        '<div class="convert-before-after-img convert-after-img-wrap"/>'
      ).appendTo ($obj),
      $divider: jQuery (
        '<div class="convert-before-after-divider">\
    ...
    this.$el.$after.children ('img').wrap ('<div class="convert-after-img"/>');
    this.$el.$after
      .children ('.convert-after-img')
      .css (
        'background-image',
...
const baw_axis = new convert_PanAxis (10);
...
class convert_Menu {
  constructor (_core = null) {
    ...
    this.core.$dom.header.append (
      '<span class="convert-active-menu-ind"></span>'
    );
    ...
    // Create Mobile Menu
    this.mobile_menu = new convert_Mobile_Menu (this.$nav);
  }
  layout () {
    const _self = this;
    let active_pos = '-100%';
...
class convert_Mobile_Menu {
  constructor ($obj) {
    ...
    this.$nav = jQuery ('<nav class="convert-mobile-nav"/>').appendTo (
      this.$body
    );
    if (this.$el.parents ('header').hasClass ('is-frosted')) {
      this.$nav.addClass ('is-frosted');
    }
    this.$menu = jQuery ('<ul class="convert-mobile-menu"/>').appendTo (
      this.$nav
    );
    ...
    this.$overlay = jQuery (
      '<div class="convert-mobile-menu-overlay"/>'
    ).appendTo (this.$body);
    ...
    // Events
    jQuery ('.convert-mobile-menu-toggler').on ('click', function () {
...
class convert_Infinite_List {
  constructor ($obj = null, cfg = {}) {
    ...
    // Hover Event
    this.$el
      .on ('mouseenter', '.convert-infinite-list-item', function () {
        ...
        if ('video' === $preview.attr ('data-type')) {
          jQuery (this)
            .children ('.convert-infinite-list-preview')
            .children ('video')[0]
            .play ();
          $preview.children ('video')[0].play ();
        }
      })
      .on ('mouseleave', '.convert-infinite-list-item', function () {
  ...
  init () {
    ...
    // DOM
    this.$list = this.$el.children ('.convert-infinite-list');
    this.$bg_wrap = jQuery (
      '<div class="convert-infinite-list-bgs"/>'
    ).appendTo (this.$el);
    this.$scroll = jQuery ('<div class="convert-infinite-scroll"/>').appendTo (
      this.$el
    );
    this.$cloned_list = jQuery (
      '<div class="convert-infinite-scroll-inner"/>'
  ...
  clone_items () {
    ...
    this.$list.children ().each (function (i) {
      ...
      if (_self.first_circle) {
        let $preview = $cloned.children ('.convert-infinite-list-preview');
        ...
        if ('video' === type) {
          ...
          _self.$bg_wrap.append (`
                    <div class="convertl-bg-preview-item${i === 0 ? ' is-active' : ''}" data-type="video" data-id="${i}" style="background-image:url(${this_src})">
                        <video src="${this_src}" webkit-playsinline="true" playsinline="true" muted loop></video>
                    </div>`);
        } else {
          $preview.css ('background-image', 'url(' + this_src + ')');
          _self.$bg_wrap.append (
            `<div class="convertl-bg-preview-item${i === 0 ? ' is-active' : ''}" data-type="image" data-id="${i}" style="background-image:url(${this_src})"/>`
          );
        }
      }
    });
...
class convert_Marquee {
  constructor ($obj = null) {
    ...
    this.$inner = this.$marquee.children ('.convert-marquee-inner');
    this.$content = this.$inner.children ();
    this.width = this.$inner.find ('.convert-marquee-list').width ();
  ...
  init () {
    this.$inner.wrapInner ('<div class="convert-marquee-inner-wrap"/>');
    this.$inner_cont = this.$inner.children ('.convert-marquee-inner-wrap');
...
class convert_Masked {
  constructor ($obj = null, cfg = {}) {
    ...
    this.cfg = Object.assign (
      {
        media_selector: '.convert-masked-media',
        content_selector: '.convert-masked-content',
        dbr: 24,
        ibr: 32,
      },
      cfg
...
class convert_LWP {
  constructor ($obj = null) {
    ...
    this.$media = jQuery (
      '<div class="convert-lwp-media" data-appear="fade-right"/>'
    ).prependTo (this.$el);
    this.count = this.$list.children ('.convert-lwp-item').length;
    this.active = 0;
    this.prev = 1;

    this.$list.children ('.convert-lwp-item').each (function (i) {
