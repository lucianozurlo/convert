...
const convert_config = {
  ...
  // Cursor Following
  cursorFollow: '.convert-socials-list a, .convert-gallery-nav',

  // Links Exception
  linksException: [
    '.comment-reply-link',
    '.shadowcore-lightbox-link',
    '.convert-lightbox-link',
    '.convert-noanim-link',
  ],
};
...
jQuery (function ($) {
  ...
  const core = new ST_Core (convert_config);
  ...
  function MainMenuInit () {
    template.menu = new convert_Menu (core);
  }

  // --- PHOTOSWITE LIGHTBOX --- //
  function initPSWP () {
    if (jQuery ('.convert-lightbox-link').length) {
      if (typeof PhotoSwipe === 'function') {
                        ...
                        <div class="pswp__scroll-wrap">\
                            ...
                            <div class="pswp__ui pswp__ui--hidden">\
                                <div class="pswp__top-bar">\
                                    ...
                                    \
                                    <button class="pswp__button pswp__button--close convert-pswp-close" title="Close (Esc)">\
                                        <i></i>\
                                    </button>\
                                    \
                                ...
                                \
                                <button class="pswp__button convert-pswp-nav convert-pswp-prev pswp__button--arrow--left" title="Previous (arrow left)"></button>\
                                <button class="pswp__button convert-pswp-nav convert-pswp-next pswp__button--arrow--right" title="Next (arrow right)"></button>\
                                \
        ...
        // Build PSWP Gallery
        jQuery ('.convert-lightbox-link').each (function () {
          ...
          if (
            ...
            if ('video' === $this.attr ('data-type')) {
              this_item.html =
                '\
                            <div class="convert-pswp-media--video" data-src="' +
                $this.attr ('href') +
                '">\
                                <video src="' +
                $this.attr ('href') +
              ...
              this_item.html =
                '\
                            <div class="convert-pswp-media--iframe" data-player-src="' +
                this_link +
                '" data-src="' +
                $this.attr ('href') +
                '" data-type="' +
        ...
        core.$dom.body.on ('click', '.convert-pswp-image-wrap', function (e) {
          e.preventDefault ();
          e.stopPropagation ();
        });
        core.$dom.body.on (
        ...
        jQuery (document).on ('click', '.convert-lightbox-link', function (e) {
          e.preventDefault ();
          if (typeof PhotoSwipe === 'function') {
            let $this = jQuery (this),
            ...
            template.pswp.lightbox.listen ('resize', function () {
              if (
                ...
                jQuery (template.pswp.lightbox.container)
                  .find ('.convert-pswp-media--video')
                  .width (template.pswp.resizeVideo ().w)
                  .height (template.pswp.resizeVideo ().h);
              }
              if (
                jQuery (template.pswp.lightbox.container).find (
                  '.convert-pswp-media--iframe'
                ).length
              ) {
                jQuery (template.pswp.lightbox.container)
                  .find ('.convert-pswp-media--iframe')
                  .width (template.pswp.resizeVideo ().w)
                  .height (template.pswp.resizeVideo ().h);
              }
            });
        ...
        console.error ('convert Error: PSWP script not found.');
      }
    }
  }
  ...
  function HeroSections () {
    // Masked Blocks
    if (jQuery ('.convert-masked-block').length) {
      jQuery ('.convert-masked-block').each (function () {
        new convert_Masked (this);
      });
    }
  }
  ...
  function BlocksHighlight () {
    if (jQuery ('.convert-backlight').length) {
      template.cursor_pos = {
        x: 0,
        y: 0,
      };
      jQuery (document).on ('mousemove', function (e) {
        ...
        jQuery ('.convert-backlight').css ({
          '--mouse-x': `${template.cursor_pos.x}px`,
          '--mouse-y': `${template.cursor_pos.y}px`,
        });
      });

      if (jQuery ('.convert-block').length) {
        jQuery ('.convert-block').each (function () {
  ...
  // --- MARQUEE ELEMENT --- //
  function Marquee () {
    if (jQuery ('.convert-marquee').length) {
      jQuery ('.convert-marquee').each (function () {
        new convert_Marquee (this);
      });
    }
  }
    ...
    if (jQuery ('.swiper').length) {
      ...
      const init_swiper = function () {
        ...
        jQuery ('.swiper').each (function () {
          ...
          if (core.$dom.body.hasClass ('convert-fullscreen-page')) {
            options.mousewheel = true;
          }
          // CAROUSEL
          if ($this.hasClass ('convert-carousel')) {
          ...
          // SLIDER
          if ($this.hasClass ('convert-slider')) {
            ...
            // Slider Content
            if ($this.find ('.convert-slide-content').length) {
              let content_mask_id =
              ...
              $this
                ...
                .each (function (i) {
                  ...
                  if ($slide.children ('.convert-slide-content').length) {
                    ...
                    $media.wrap ('<div class="convert-slider-media"/>');
          ...
          // Pagination
          if ($this.children ('.convert-dots').length) {
            options.pagination = {
              el: '.convert-dots',
              dynamicBullets: true,
          ...
          // Slider Navigation
          if ($this.parent ().children ('.convert-slider-nav').length) {
            let $this_nav = $this.parent ().children ('.convert-slider-nav');
            options.navigation = {
              prevEl: $this_nav.children ('.convert-slider-prev')[0],
              nextEl: $this_nav.children ('.convert-slider-next')[0],
            };
      ...
      if (typeof Swiper === 'function') {
        ...
        console.error ('convert Error: Swiper Library not found.');
      }
    }
  }
  ...
  function CircleLinks () {
    if (jQuery ('.convert-circle-path-link:not(.is-init)').length) {
      jQuery ('.convert-circle-path-link:not(.is-init)').each (function () {
        let $this = jQuery (this),
  ...
  function LinksEvents () {
    jQuery ('.convert-back-to-listing').on ('click', function () {
    ...
    if (
      jQuery ('.convert-albums-listing').length &&
      window.localStorage.getItem ('convert_return_from') != false
    ) {
      jQuery (window).trigger (
        'convert_set_album',
        window.localStorage.getItem ('convert_return_from')
      );
      window.localStorage.setItem ('convert_return_from', 0);
    } else {
      window.localStorage.setItem ('convert_return_from', 0);
    }
  }
  ...
  function InfiniteList () {
    if (jQuery ('.convert-infinite-list-wrap:not(.is-init)').length) {
      jQuery ('.convert-infinite-list-wrap:not(.is-init)').each (function () {
        this.classList.add ('is-init');
        core.updateRequired.push (new convert_Infinite_List (this));
      });
    }
  }
  ...
  // --- COUNTERS --- //
  function Counters () {
    if (jQuery ('.convert-counter:not(.is-init)').length) {
      jQuery ('.convert-counter:not(.is-init)').each (function () {
        let $this = jQuery (this);
        this.classList.add ('is-init');
        this.counter_count = function () {
          let $counter = $this.children ('.convert-counter-number');
  ...
  function ListWithPreview () {
    if (jQuery ('.convert-list-with-preview').length) {
      jQuery ('.convert-list-with-preview').each (function () {
        new convert_LWP (this);
      });
    }
  }
  ...
  function BeforeAfter () {
    if (jQuery ('.convert-before-after:not(.is-init)').length) {
      jQuery ('.convert-before-after:not(.is-init)').each (function () {
        jQuery (this).addClass ('is-init');
        core.updateRequired.push (new convert_Before_After (jQuery (this)));
      });
    }
  }
  ...
  // --- TOGGLES --- //
  function Toggles () {
    if (jQuery ('.convert-toggles-item:not(.is-init)').length) {
      jQuery ('.convert-toggles-item:not(.is-init)').each (function () {
        let $this = jQuery (this);
        $this.addClass ('is-init');
        $this
          .children ('.convert-toggles-item--title')
          .append ('<i class="convert-icon convert-icon-arrow-down"/>');
        $this.children ('.convert-toggles-item--content').slideUp (1);
        $this.on ('click', function () {
          jQuery (this).toggleClass ('is-active');
          $this.children ('.convert-toggles-item--content').slideToggle (300);
        });
        $this.on ('click', '.convert-toggles-item--content', function (e) {
          e.stopPropagation ();
          e.preventDefault ();
        });
      });
  ...
  function ScrollTrigger () {
    ...
    if (jQuery ('.convert-inline-ribbon:not(.is-init)').length) {
      jQuery ('.convert-inline-ribbon:not(.is-init)').each (function () {
        if (core.sap !== null) {
          let $this = jQuery (this);
          $this.addClass ('is-init');
    ...
    // Sticky Elements
    if (jQuery ('.convert-sticky-block:not(.is-init)').length) {
      jQuery ('.convert-sticky-block:not(.is-init)').each (function () {
        if (core.sap !== null) {
          let $this = jQuery (this);
          $this.addClass ('is-init');
    ...
    // Expand on Scroll
    if (jQuery ('.convert-expand-on-scroll:not(.is-init)').length) {
      jQuery ('.convert-expand-on-scroll:not(.is-init)').each (function () {
        if (core.sap !== null) {
          let $this = jQuery (this);
          $this.addClass ('is-init');
    ...
    // Text Mask Fill
    if (jQuery ('.convert-text-fill-mask:not(.is-init)').length) {
      jQuery ('.convert-text-fill-mask:not(.is-init)').each (function () {
        if (core.sap !== null) {
          let $this = jQuery (this);
          $this.addClass ('is-init');
    ...
    // Parallax Media
    if (jQuery ('.convert-parallax-media:not(.is-init)').length) {
      jQuery ('.convert-parallax-media:not(.is-init)').each (function () {
        let $this = jQuery (this);
        if (
  ...
  function ContactForm () {
    if (jQuery ('.convert-contact-form').length) {
      // The code related to the contact form is in the file: js/contact_form.js
      if (typeof convert_Contact_Form === 'function') {
        convert_Contact_Form ();
      } else {
        console.error ('convert Error: Contact form script not found.');
      }
    }
  }
  ...
  function InitAnimation () {
    ...
    template.checkURL = function (el) {
      ...
      if (this_href.indexOf ('javascript') === 0) {
        ...
        jQuery (convert_config.linksException).each (function () {
          if ($el.is (this)) {
            result = false;
          }
        });
  ...
  function Layout () {
    ...
    core.$dom.nav
      .find ('.convert-menu-offset')
      .removeClass ('convert-menu-offset');
    core.$dom.nav.find ('.sub-menu').each (function (i) {
      ...
      if (this_left_full > core.$dom.win.width () - 20) {
        $this.addClass ('convert-menu-offset');
      }
    });
    ...
    if (core.$dom.body.hasClass ('convert-fullscreen-page')) {
      ...
      core.$dom.main.removeClass ('convert-min-content');
      ...
      if (core.$dom.main.height () < minHeight) {
        core.$dom.main.addClass ('convert-min-content');
  ...
  $ (document).ready (function () {
    // General UI
    RightClickProtection ('.convert-rcp-wrap');
    MainMenuInit ();
    CircleLinks ();
